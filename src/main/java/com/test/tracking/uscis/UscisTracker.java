...
public class UscisTracker {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        try {
            FileUtil.appendFile(HISTORY_FILE,
                    "\r\n*****************************************************************************************************\r\n");
            FileUtil.appendFile(HISTORY_FILE, "Running the report " + new Date().toString());
            FileUtil.appendFile(HISTORY_FILE,
                    "\r\n*****************************************************************************************************\r\n");
    ...
    private void getLatestApproved() {
        ...
        while (track < endNumber) {
            ...
            FileUtil.appendFile(TRACKING_FILE,
                    "\r\n*****************************************************************************************************\r\n");
            FileUtil.writeFile(TRACKING_FILE,
                    String.format("%-15s %-15s %-15s %-15s %-15s %-15s %-15s %s", "approved", "received",
                            "queried", "queryAnswered", "delivered", "transferred", "other", "\r\n"));
            FileUtil.appendFile(TRACKING_FILE,
                    "---------------------------------------------------------------------------------------------------------------------------------------------\r\n");
            FileUtil.appendFile(TRACKING_FILE, String.format("%-15d %-15d %-15d %-15d %-15d %-15d %-15d  %s",
                    approved, received, queried, queryAnswered, delivered, transferred, other, "\r\n"));
    ...
    private String getStatus(Integer track) {
        ...
        // record.append(date).append(" ").append(PREFIX_CENTER).append(track).append(" ").append(formType).append("
